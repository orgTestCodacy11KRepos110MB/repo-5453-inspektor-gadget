...
RUN cd /gadget/gadget-container && \
	...
	DEBUG=1 make -j$(nproc) TARGET_ARCH=${TARGETARCH} gadget-container-deps
...
# This will be used by dlv server.
ENV DEBUG_PORT=4040

# We need to install delve.
RUN set -ex; \
	export DEBIAN_FRONTEND=noninteractive; \
	apt-get update && \
	apt-get install -y --no-install-recommends \
		ca-certificates curl jq wget xz-utils binutils rpm2cpio cpio software-properties-common && \
